// This is a modified copy of iced_aw/src/native/grid.rs from commit f7c6166c9c8de6f49840308f6971843cc42e9530
// where inter-cell spacing is added to the grid widget.
    ...
    Renderer: iced_native::Renderer,
{
    ...
    pub fn horizontal_spacing(mut self, amount: impl Into<Pixels>) -> Self {
        self.horizontal_spacing = amount.into().0;
        self
    }
    ...
    pub fn vertical_spacing(mut self, amount: impl Into<Pixels>) -> Self {
        self.vertical_spacing = amount.into().0;
        self
    }
            ...
            Strategy::Columns(columns) => {
                ...
                let column_aligns =
                    std::iter::once(&0.)
                        ...
                        .scan(0., |state, width| {
                            *state += width + self.horizontal_spacing;
                            Some(*state)
                        });
...
fn build_grid(
    ...
    for ((column, column_align), mut node) in (0..columns).zip(column_aligns).cycle().zip(layouts) {
        if column == 0 {
            grid_height += row_height + vertical_spacing;
            row_height = 0.;
        }
